@extends('layouts.app')
@section('heading', 'New Quote')
@section('content')
<div class="max-w-3xl" x-data="quoteBuilder()">
    <form method="POST" action="{{ route('quotes.store') }}" class="bg-slate-900 rounded-xl border border-slate-800 p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-5">
            <div x-data="clientAutocomplete()" class="relative">
                ...
                <input type="text" name="client_name" x-model="query" @input.debounce.300ms="search()" @focus="open = suggestions.length > 0" @click.away="open = false" value="{{ old('client_name', $enquiry?->client_name) }}" required autocomplete="off" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-brand-500">
            ...
            <div>
                ...
                <select name="enquiry_id" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-brand-500">
                    ...
                    @foreach($enquiries as $e)
                        <option value="{{ $e->id }}" {{ old('enquiry_id', $enquiry?->id) == $e->id ? 'selected' : '' }}>{{ $e->reference }} — {{ $e->client_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="scope_of_work" rows="3" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-brand-500">{{ old('scope_of_work') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-3 gap-5">
            ...
            <div>
                ...
                <input type="date" name="valid_until" value="{{ old('valid_until') }}" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-400 mb-1.5">Payment Terms</label>
                <input type="text" name="terms" value="{{ old('terms', 'Net 30') }}" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-brand-500">
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="px-5 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700 transition">Create Quote</button>
            <a href="{{ route('quotes.index') }}" class="px-4 py-2 text-slate-400 text-sm hover:text-slate-200">Cancel</a>
        </div>
    </form>
</div>
...
function quoteBuilder() {
    return {
        taxRate: {{ old('tax_rate', 13) }},
        items: [{ description: '', quantity: 1, unit: 'hours', rate: 150 }],
...
function clientAutocomplete() {
    return {
        query: '{{ old("client_name", $enquiry?->client_name ?? "") }}',
        suggestions: [],
        open: false,
        async search() {
